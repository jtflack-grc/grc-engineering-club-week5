...
FINDINGS_FILE="${1:-evidence/security-hub-findings.json}"
...
if [[ ! -f "$HASH_FILE" && -f "security-hub-findings.json.sha256" ]]; then
  HASH_FILE="security-hub-findings.json.sha256"
fi

SIG_BUNDLE="${FINDINGS_FILE%.json}.sig.bundle"
if [[ ! -f "$SIG_BUNDLE" && -f "security-hub-findings.sig.bundle" ]]; then
  SIG_BUNDLE="security-hub-findings.sig.bundle"
fi

if [[ ! -f "$HASH_FILE" ]]; then
  echo "VERIFY FAILED: hash sidecar not found. Expected ${FINDINGS_FILE}.sha256 or security-hub-findings.json.sha256" >&2
  exit 1
fi

if [[ ! -f "$SIG_BUNDLE" ]]; then
  echo "VERIFY FAILED: signature bundle not found. Expected ${FINDINGS_FILE%.json}.sig.bundle or security-hub-findings.sig.bundle" >&2
  exit 1
fi
...
echo "   OK: Cosign verified the Security Hub findings evidence."
echo "CHAIN INTACT"
